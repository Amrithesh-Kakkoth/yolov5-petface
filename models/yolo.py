...
from models.common import Conv, Bottleneck, SPP, DWConv, Focus, BottleneckCSP, C3, ShuffleV2Block, Concat, NMS, autoShape, StemBlock, BlazeBlock, DoubleBlazeBlock, SPPFSmall
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.nkpt = 3  # number of keypoints (left_eye, right_eye, mouth)
        self.no = nc + 5 + self.nkpt * 2  # number of outputs per anchor
    ...
    def forward(self, x):
        ...
        if self.export_cat:
            for i in range(self.nl):
                ...
                cls_start = 5 + self.nkpt * 2
                y = y + torch.cat((x[i][:, :, :, :, 0:5].sigmoid(),
                                   torch.cat((x[i][:, :, :, :, 5:cls_start],
                                              x[i][:, :, :, :, cls_start:cls_start + self.nc].sigmoid()), 4)), 4)
                ...
                y = torch.cat([box_xy, box_wh, y[:, :, :, :, 4:5], landm1, landm2, landm3,
                               y[:, :, :, :, cls_start:cls_start + self.nc]], -1)
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = torch.full_like(x[i], 0)
                cls_start = 5 + self.nkpt * 2
                class_range = list(range(5)) + list(range(cls_start, cls_start + self.nc))
                y[..., class_range] = x[i][..., class_range].sigmoid()
                y[..., 5:cls_start] = x[i][..., 5:cls_start]
                #y = x[i].sigmoid()
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            s = 256  # 2x min stride (increased to avoid degenerate 1x1 with Stem+P6)
